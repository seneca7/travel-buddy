...
- **Scorer not yet wired into the UI.** `Scorer.score(...)` exists and is tested, but the Matches feed still renders three hardcoded `MatchProfile` rows (now owned by `InMemoryRepository.DEFAULT_SAMPLE_MATCHES`). The wiring lands when Sprint B replaces `InMemoryRepository` with a `FirestoreRepository` and the UI starts consuming `Profile` + `Trip` directly.
...
2. ✅ **Introduce a `Repository` interface + `InMemoryRepository` impl.** Done. `repository/Repository.kt` is the swappable abstraction; `repository/InMemoryRepository.kt` owns trip draft, published trip, sample matches, and pending chat preview. `TravelViewModel` refactored to consume the interface via a default-arg constructor, so Compose's `viewModel()` keeps working without a factory. `InMemoryRepositoryTest` (11 cases) locks the validation + truncation + clock-injection invariants. Sprint B's `FirestoreRepository` drops in behind the same interface without touching the ViewModel.
